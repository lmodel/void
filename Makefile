...
ifneq ($(strip ${GEN_TARGS}),)
	...
	$(RUN) gen-typescript ${GEN_TARGS} $(SOURCE_SCHEMA_PATH) >${DEST}/typescript/void.ts
endif

#### To workaround https://github.com/linkml/linkml/issues/1453
#### you can trigger generation using config.env
ifneq ($(strip ${GEN_OARGS}),)
	$(RUN) gen-owl ${GEN_OARGS} -o $(DEST)/owl/void.owl.ttl $(SOURCE_SCHEMA_PATH)
endif
...
examples/output: src/void/schema/void.yaml
	mkdir -p $@
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/void/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py project config.env config.yaml
